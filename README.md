...
- **静的HTML (ケース 1 & 4)**:
  ...
  npm run build      # 完成時に実行
  ```
- **動的PHP (ケース 2, 3 & 5)**: ローカルサーバーを起動してから:
  ```bash
  npm run link       # 初回のみ
  npm run dev        # 開発開始
  npm run build      # 完成時に実行
...
### ⚙️ 環境の設定方法:
1. **GitHub Secrets**: `Settings > Secrets and variables > Actions` に移動します。`deploy-config.json` 内の `"server"` に指定した値（例: `"JLWEB_SSH"`）に基づき、`[サーバー名]_CONFIG` という名前のシークレット（例: `JLWEB_SSH_CONFIG`）を作成し、以下のJSONを入力します:
   ```json
   {
   ...
   *各項目の説明:*
   - `"host"`: サーバーアドレス.
   - `"user"` / `"pass"`: ログイン情報 (`deploy_method` が `ssh` または `ftp` の場合に使用).
   - `"ftp_dir"`: デプロイ先のエンドポイントとなる相対ディレクトリパス.
   - `"root_path"`: サーバー上の物理絶対パス（`.htaccess`ファイルのフルパス指定等で使用します）.
2. **`deploy-config.json`**: ルートレベルで展開先の親ディレクトリ (`project_dir`) などを設定し、各ブランチごとの展開方法 (`deploy_method`) や認証を設定します:
   ```json
   {
   ...
   ```
   *注意:*
   *- `"deploy_method"` には `"ftp"`, `"ssh"`, `"zip"` を指定可能です。*
   *- `"ssh"` または `"ftp"` メソッドで `"basic_auth"` を設定すると、自動的に `.htaccess`と`.htpasswd`が生成されサーバーにベーシック認証がかかります。*
   *- **ZIP納品モード**: `"deploy_method"` を `"zip"` に変更するとサーバー通信をスキップし、独立した納品用`.zip`ファイルのみをGitHub Releasesに自動生成します。*
...
- **Nhóm HTML Tĩnh (Trường hợp 1 & 4)**:
  ```bash
  npm run dev        # Chạy dev
  npm run build      # Khi chốt dự án
  ```
- **Nhóm PHP Động (Trường hợp 2, 3 & 5)**: Bật Server ảo, sau đó:
  ```bash
  npm run link       # Chỉ chạy 1 lần đầu
  npm run dev        # Chạy dev
  npm run build      # Khi chốt dự án
...
1. **GitHub Secrets**: Vào mục `Settings > Secrets and variables > Actions`. Dựa theo tên bạn đặt ở mục `"server"` trong `deploy-config.json` (VD: `"JLWEB_SSH"`), hãy tạo một hàm biến Secret theo đúng cú pháp `[TÊN-SERVER]_CONFIG` (Ví dụ: `JLWEB_SSH_CONFIG`). Value bên trong nhúng nguyên 1 khối JSON sau:
   ```json
   {
     "host": "your.server.com",
     "user": "username",
   ...
   *Giải thích thông số JSON:*
   - `"host"`: Địa chỉ truy cập máy chủ.
   - `"user"` / `"pass"`: Tài khoản và Mật khẩu (Dùng chung cho `ftp` hoặc `ssh`).
   - `"ftp_dir"`: Đường dẫn trỏ tới thư mục đích trên cây thư mục.
...
2. **`deploy-config.json`**: Tại cấp cao nhất (root), khai báo thư mục xuất web (`source_folder`) và thư mục đích (`project_dir`). Sau đó, cấu hình phương thức (FTP/SSH/ZIP) cho từng nhánh:
   ```json
   {
     "source_folder": "public",
     "project_dir": "template_test_ssh",
   ...
   ```
   *Ghi chú quan trọng:*
   *- `"deploy_method"` có thể nhận 1 trong 3 giá trị là `"ftp"`, `"ssh"`, hoặc `"zip"`.*
   *- Nếu khai báo block `"basic_auth"` kèm giao thức `ssh` hoặc `ftp`, hệ thống sẽ TỰ ĐỘNG sinh ra 2 file `.htaccess` và `.htpasswd` đính kèm khi upload để khóa thư mục bảo mật bằng mật khẩu.*
   *- **Chế độ Bàn Giao Zip**: Khi gán `"deploy_method": "zip"`, hệ thống sẽ bỏ qua bước kết nối Server và tự động Build ra 1 file nén `.zip` sạch sẽ đưa vào tab GitHub Releases (lúc này `"server"` có thể để trống).*
